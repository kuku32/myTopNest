...
export class TopMangaService {
  ...
  async postUsercheckView(type:string,chapter:string, data:any){
    try{
      let BASE_URL = `${this.configService.get<any>('FIREBASE_DATA')}/checkviews/${type}/${chapter}/${data?.uuid}.json`;
      const response = await axios.put(BASE_URL, data);
      return response?.data
    }catch(error){
      throw new UnauthorizedException("You're not allow");
  ...
  async PostUserInteraction(action:string,type:string,chapter:string, data:any){
    try{
      let BASE_URL = `${this.configService.get<any>('FIREBASE_DATA')}/userinteraction/${action}/${type}/${chapter}/${data?.uuid}.json`;
      let response
      if(chapter==='view'){
        response = await axios.post(BASE_URL, data);
      }else{
        response = await axios.put(BASE_URL, data);
      }
